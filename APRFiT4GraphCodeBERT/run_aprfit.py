...
def main():
    ...
    parser.add_argument("--scheduler_fun", default="S5", type=str)
    ...
    if args.do_train:
        ...
        initial = 52364 / len(train_dataset)
        ...
        train_sampler_temp = RandomSampler(train_dataset)
        train_dataloader_temp = DataLoader(train_dataset, sampler=train_sampler_temp, batch_size=args.train_batch_size//args.gradient_accumulation_steps,num_workers=4)
        ...
        scheduler = get_linear_schedule_with_warmup(optimizer, num_warmup_steps=len(train_dataloader_temp)*args.num_train_epochs*0.1,num_training_steps=len(train_dataloader_temp)*args.num_train_epochs)
        ...
        for epoch in range(args.num_train_epochs):
            ...
            curriculum = args.num_train_epochs * len(train_dataloader_temp)
            training_scheduler = min(1, SCHEDULER_FUN[args.scheduler_fun](progress, curriculum, initial))
            ratio = int(training_scheduler * len(train_dataset))
            if ratio != current_ratio:
                current_ratio = ratio
                train_data = train_dataset[0:current_ratio]
                train_sampler = RandomSampler(train_data)
                train_dataloader = DataLoader(train_data, sampler=train_sampler, batch_size=args.train_batch_size//args.gradient_accumulation_steps,num_workers=4)
